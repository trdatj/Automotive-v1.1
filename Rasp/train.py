...
# 1. Cấu hình hệ thống cho Raspberry Pi
...
# 3. Tính toán class weights để xử lý mất cân bằng dữ liệu
class_weights = compute_class_weight('balanced',
                                   classes=np.unique(train_generator.classes),
                                   y=train_generator.classes)
class_weights = dict(enumerate(class_weights))

# 4. Xây dựng mô hình tối ưu cho Pi
def create_model():
...
# 5. Callbacks để tối ưu quá trình training
callbacks = [
...
EPOCHS = 30  # Tăng epoch nhưng có early stopping
...
# 7. Chuyển đổi sang TensorFlow Lite và quantization
